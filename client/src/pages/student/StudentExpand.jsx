...
const InternshipExpanded = ({ internship }) => {
  ...
  const handleOfferLetterUpload = async (e) => {
    ...
    try {
      ...
      const res = await uploadOfferLetter(internship._id, formData);
  ...
  const handleCompletionUpload = async (e, documentType) => {
    const file = e.target.files[0];
    ...
    try {
      ...
      const res = await uploadCompletionDocument(internship._id, formData);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="bg-white border rounded-xl p-5">
        <h3 className="font-bold text-slate-800 mb-4">Company Details</h3>
      ...
      {/* Teacher Details */}
      <div className="bg-white border rounded-xl p-5">
        <h3 className="font-bold text-slate-800 mb-4">Teacher Details</h3>
      ...
      {/* NOC */}
      <div className="bg-white border rounded-xl p-5">
        <h3 className="font-bold text-slate-800 mb-4">NOC Details</h3>
          ...
          {internship.completionDocuments?.report?.url ? (
            ...
            <label className="cursor-pointer text-blue-600">
              ...
              <input
                ...
                accept=".pdf"
                className="hidden"
                onChange={(e) => handleCompletionUpload(e, "report")}
              />
            </label>
          )}
        </div>
          ...
          {internship.completionDocuments?.certificate?.url ? (
            ...
            <label className="cursor-pointer text-blue-600">
              ...
              <input
                ...
                accept=".pdf"
                className="hidden"
                onChange={(e) => handleCompletionUpload(e, "certificate")}
              />
            </label>
          )}
        </div>
          ...
          {internship.completionDocuments?.ppt?.url ? (
            ...
            <label className="cursor-pointer text-blue-600">
              ...
              <input
                ...
                accept=".ppt,.pptx,.pdf"
                className="hidden"
                onChange={(e) => handleCompletionUpload(e, "ppt")}
              />
            </label>
          )}
        </div>
