import { Upload, Download, Clock, CheckCircle } from "lucide-react";

const InternshipActions = ({ internship }) => {
  const offerUploaded = !!internship.externalDetails?.offerLetter;
  ...
  const nocIssued = internship.noc?.status === "Issued";

  const teacherAssigned = !!internship.teacherAssignment?.teacher;

  const reportUploaded = !!internship.completionDocuments?.report?.url;

  const certificateUploaded =
    !!internship.completionDocuments?.certificate?.url;

  const pptUploaded = !!internship.completionDocuments?.ppt?.url;
  ...
  return (
    <div className="flex flex-wrap gap-3 mt-6">
      ...
      {teacherAssigned && internship.status === "Internship Ongoing" && (
      ...
      {reportUploaded && certificateUploaded && pptUploaded && (
        <div
          className="
        flex items-center gap-2
        bg-green-100
