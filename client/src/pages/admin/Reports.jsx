...
const downloadExcel = () => {
  ...
  XLSX.writeFile(
    workbook,
    "internship-reports.xlsx"
  );

};
const downloadPDF = () => {
   console.log(report.noc);
  if (!filteredReports.length) {
    alert("No reports available");
    return;
  }
...
const generateNOC = async (data) => {
 ...
 console.log("🚀 GENERATE NOC CLICKED", data);
  try {
...
console.log("NOC PAYLOAD:", nocData);
...
const handleApprove = async (report) => {
  console.log("Approve clicked:", report);
...
const filteredReports = (reports || []).filter((report) => {
  ...
  const departmentMatch =
    !selectedDepartment ||
    report.department?._id === selectedDepartment;

  const sessionMatch =
    !selectedSession ||
    report.academicSession?._id === selectedSession;
...
setReports(internships);
    ...
    setStats({
      ...
      teachers: new Set(
        internships
        .filter(
          i=>i.teacherAssignment?.teacher
        )
        .map(
          i=>i.teacherAssignment.teacher._id
        )
      ).size,
...
const departmentChartData = Object.values(
  filteredReports.reduce((acc, report)=>{
    ...
    const dept =
      report.department?.departmentName || "Unknown";
...
const domainChartData = Object.values(
  filteredReports.reduce((acc, report)=>{
    ...
    const domain =
      report.domain?.domainName || "Unknown";
